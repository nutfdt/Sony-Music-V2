...
/*Création d'un admin, cet utilisateur nexiste que dans la table User*/
...
/*Création de lalbum pour l'artiste créé et dune chanson*/
...
drop view if exists totalArtistesAgent;
create view totalArtistesAgent as (
select ag.iduser, count(a.iduser) from agent ag, artiste a where ag.iduser=a.idagent group by ag.iduser
);

drop view if exists vueArtistesAlbums;
create view vueArtistesAlbums as (
    select ar.iduser,count(a.idalbum) from album a, artiste ar, agent as ag
    where ar.iduser=a.idartiste and ar.idagent=ag.iduser
    group by ar.iduser
);